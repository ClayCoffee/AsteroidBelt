...
public class DataYML {
	...
	public void reloadCustomConfig() {
		if (configFile == null) {
			configFile = new File(AsteroidBelt.instance.getDataFolder(), configName);
		}
		config = YamlConfiguration.loadConfiguration(configFile);

		Reader stream;
		try {
			stream = new InputStreamReader(AsteroidBelt.instance.getResource(configName), "UTF8");
			if (stream != null) {
	...
	public void saveCDefaultConfig() {
		...
		if (!configFile.exists()) {
			...
			AsteroidBelt.instance.saveResource(configName, false);
		}
	}
}
